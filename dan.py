#Author: KSHITIJ KUMAR SRIVASTAVA (02091785)
# Date: OCT 20, 2023
import argparse
import torch
...
class QuestionDataset(Dataset):
    ...
    def vectorize(ex, word2ind):
        ...
        ####Your code here
        # Iterate through the words in the tokenized question text

        for i in range(len(ex)):
            w = ex[i]
            if w in word2ind:
                vec_text[i] = word2ind[w]
            else:
...
class DanModel(nn.Module):
    ...
    def __init__(self, n_classes, vocab_size, emb_dim=50,
        ...
        #### Your code here
        # Adding the dropout layer with the given dropout parameter
    ...
    def forward(self, input_text, text_len, is_prob=False):
        ...
        embed = self.embeddings(input_text)
        # Then average them 
        avg_embed = embed.sum(dim=1) / text_len.view(embed.size(0), -1).float()
        # Before feeding them through the network
        logits = self.classifier(avg_embed)
        if is_prob:
